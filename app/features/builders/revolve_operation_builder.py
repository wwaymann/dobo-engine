...
class RevolveOperationBuilder(FeatureOperationBuilder[RevolveFeatureDefinition]):
    ...
    @property
    def feature_type(self) -> type[RevolveFeatureDefinition]:
        return RevolveFeatureDefinition

    def build(self, feature: RevolveFeatureDefinition, context: FeatureContext) -> FeaturePlan:
        ...
        request = GeometryRequest(
            ...
            geometry=geometry,
            operation=GeometryOperationType.REVOLVE,
            parameters={
                "angle": float(feature.angle),
                "axis_origin": tuple(float(v) for v in feature.axis_origin),
                "axis_direction": feature.normalized_axis_direction,
            },
            output_id=tool_id,
        ...
        geometry_operation = GeometryOperation(
            ...
            request=request,
            output_id=tool_id,
            tags=("feature", "revolve", feature.mode.value),
            metadata={"feature_id": feature.id, **feature.metadata},
        )
        ...
        plan = FeaturePlan(
            ...
            metadata={"builder": "revolve_operation_builder"},
        )
        plan.validate()
        return plan
