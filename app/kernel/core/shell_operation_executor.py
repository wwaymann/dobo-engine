...
class ShellOperationExecutor(
    ...
    def _execute(
        ...
        if not faces:
            raise RuntimeError("Shell source geometry contains " "no faces.")
        ...
        for index in operation.remove_face_indices:
            ...
            if not isinstance(
                ...
                raise TypeError(
                    "Shell face selection did not " "produce a CadQuery Face."
                )

            remove_faces.append(face)

        if not remove_faces:
            raise ValueError("ShellOperation requires at least " "one face to remove.")

        try:
            workplane = cq.Workplane(obj=geometry)

            selected_faces = workplane.newObject(remove_faces)

            shelled_workplane = selected_faces.shell(
                float(operation.thickness),
                kind="arc",
            )

            shelled_value = shelled_workplane.val()

        except Exception as error:
            raise RuntimeError(
                "ShellOperationExecutor could not " "generate the shell."
            ) from error
        ...
        if not isinstance(
            ...
            raise RuntimeError("Shell operation did not produce " "a CadQuery Shape.")

        shelled = shelled_value.clean()
        ...
        solid = SolidFactory.from_shape(
            ...
            metadata={
                ...
                "removed_face_count": len(remove_faces),
                **operation.metadata,
            },
        )
        ...
        context.log(
            level="info",
            message=("Shell operation generated " f"'{operation.output_id}'."),
