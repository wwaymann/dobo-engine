...
# Primitive pot audit trigger: execute existing consolidated behavior only.
import json
import os
import shutil
...
def run(output_root: str|Path="outputs-ci/primitive-pot-benchmark")->dict:
    ...
    for case in CASES:
        ...
        r={"id":case["id"],"label":case["label"],"status":"FAIL","stage":"start","error":None,"semantic_family":None,"style_tags":[],"body_profile":None,"morphology_profile":None,"family_match":False,"opening_valid":False,"watertight":False,"winding_consistent":False,"production_size_valid":False,"three_mf_valid":False,"renders":{}}
        try:
            r["stage"]="semantic"; interpreted=interpreter.interpret(case["prompt"]); semantic=interpreted.program; semantic.validate(); r["semantic_family"]=semantic.body.family; r["style_tags"]=list(semantic.body.style_tags); r["opening_valid"]=semantic.body.opening_shape in {"circular","elliptical","polygonal"} and .15<=semantic.body.opening_width_ratio<=.95 and .15<=semantic.body.opening_depth_ratio<=.95
            r["stage"]="surface_bridge"; intents=SemanticSurfaceIntentBridge.compile(semantic); r["stage"]="physical_generation"; result=pipeline.generate_from_semantic(semantic,output_root=d/"generated",interpreter_version=interpreted.trace.interpreter_version,model=interpreted.trace.model,response_id=interpreted.trace.response_id,surface_intents=intents); result.validate(); r["body_profile"]=result.trace.body_profile; motor=json.loads(Path(result.motor_path).read_text(encoding="utf-8")); r["morphology_profile"]=motor.get("morphogenesis",{}).get("profile")
            ...
            physical=all((r["opening_valid"],r["watertight"],r["winding_consistent"],r["production_size_valid"],r["three_mf_valid"],Path(r["renders"]["top"]).is_file(),Path(r["renders"]["iso"]).is_file())); r["status"]="PASS" if physical and r["family_match"] else ("PARTIAL" if physical else "FAIL"); r["stage"]="complete"
