...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the DOBO Blocks 7-8 real acceptance matrix."
    )
...
def main() -> None:
    ...
    print()
    print("DOBO Macroblock A Real Matrix - Blocks 7-8")
    print("-----------------------------------")
    for case in cases:
        result = DoboStructuralPipeline().generate_from_semantic(
            case.program,
            output_root=root / case.id,
            interpreter_version="macroblock-a-matrix",
            model="not-used",
        ...
        print(case.label, "negative volumes", report.negative_volumes, "OK")
        print(case.label, "surface layers", result.trace.surface_layers, "OK")
        print(case.label, "color zones", result.trace.color_zones, "OK")
    ...
    print("DOBO Macroblock A Real Matrix: Valid OK")
