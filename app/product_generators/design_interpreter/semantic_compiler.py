...
class SemanticToMotorCompiler:
    ...
    def compile(cls, program: DesignSemanticProgram) -> SemanticCompilationResult:
        ...
        minimum_blend_mm = max(0.4, 0.4 * manufacturing.minimum_feature_mm)
        default_relief_depth_mm = min(0.8, max(0.1, 0.5 * manufacturing.maximum_relief_depth_mm))
        minimum_relief_depth_mm = min(
            0.8,
            min((feature.size.depth_mm for feature in program.features), default=default_relief_depth_mm),
        )
        ...
        for feature in program.features:
            ...
            roots.append(node)
            traces.append(CompilationTrace(feature.id, template_id, feature.id, str(template["kind"]), str(template["operation"])))

        ignored = {
            tuple(sorted((relation.subject_id, relation.object_id)))
            for relation in program.relations
            if relation.kind in {"attached_to", "above", "aligned_with", "below", "centered_on", "contained_by", "grouped_with"}
        }
        parents = {
        ...
        uses_multilevel_relations = any(relation.kind in {"attached_to", "contained_by"} for relation in program.relations)
        if uses_multilevel_relations:
            ...
            for feature_id in set(parents) | set(parents.values()):
                members_by_root.setdefault(hierarchy_root(feature_id), []).append(feature_id)
        ...
        voxel_mm = min(cls._voxel_size(width, depth, height), wall / 3.0)
        motor_program: dict[str, Any] = {
            "id": motor_id,
            "grid": {
                "minimum": [-0.5 * width - margin, -0.5 * depth - margin, base_z - margin],
                "maximum": [0.5 * width + margin, 0.5 * depth + margin, 0.5 * height + margin],
                "voxel_mm": voxel_mm,
            },
            "fields": cls._body_fields(program),
            "composition": {"field_ids": ["body", "front_mass"], "blend_mm": max(4.0, 0.075 * min(width, depth))},
            "vessel": {
                "wall_mm": wall,
                "base_z_mm": base_z,
            ...
            "hierarchy_program": {
                "adaptive_refinement": {
                    "surface_band_mm": 1.2, "size_band_ratio": 0.45, "maximum_band_mm": 2.4,
                    "small_feature_threshold_mm": max(3.0, 2.5 * manufacturing.minimum_feature_mm),
                    "detail_subdivision_passes": 1,
                },
                "proportional_scaling": {
                    "reference_radius_mm": 0.35 * width, "reference_height_mm": opening_z - base_z,
                    "minimum_scale": 0.8, "maximum_scale": 1.3, "scale_depth": False,
                },
                "layout_constraints": {
            ...
            "output": {
                "directory": "outputs/product_generators/design_interpreter/" f"{motor_id}",
                "basename": motor_id,
    ...
    def _body_fields(program: DesignSemanticProgram) -> list[dict[str, Any]]:
        ...
        return [
            {"id": "body", "center": [0.0, 0.0, 0.0], "radii": [width_radius, depth_radius, height_radius]},
            {"id": "front_mass", "center": [0.0, -0.16 * body.depth_mm, 0.02 * body.height_mm], "radii": [front_factor * width_radius, 0.78 * depth_radius, 0.88 * height_radius]},
        ]

    @staticmethod
    def _compile_anchor(anchor: SemanticAnchor, feature: FeatureIntent) -> dict[str, float]:
        region_center = {"front": 0.0, "back": 180.0, "left": -90.0, "right": 90.0, "upper": 0.0, "lower": 0.0, "all_around": 0.0}[anchor.region]
        ...
        return {
            ...
            "height_ratio": height,
            "offset_mm": 0.15 * feature.size.depth_mm if feature.surface_effect in {"raised", "marking"} else 0.0,
            "roll_degrees": anchor.roll_degrees,
        }

    @classmethod
    def _compile_feature(
        cls, feature: FeatureIntent, *, template_id: str, body_width_mm: float,
        body_height_mm: float, minimum_feature_mm: float, minimum_blend_mm: float,
        minimum_relief_depth_mm: float, maximum_relief_depth_mm: float,
    ) -> tuple[dict[str, Any], dict[str, Any] | None]:
        width = max(feature.size.width_ratio * body_width_mm, minimum_feature_mm)
        height = max(feature.size.height_ratio * body_height_mm, minimum_feature_mm)
        depth = feature.size.depth_mm
        operation = "subtract" if feature.surface_effect in {"recessed", "cutout"} else "add"
        minimum_compiled_blend = max(0.5, 0.5 * minimum_feature_mm)
        blend = max(minimum_compiled_blend, min(width, height, depth) * 0.35)
        base: dict[str, Any] = {"id": template_id, "operation": operation, "blend_mm": blend, "probe": [0.0, 0.0, 0.0]}
        transform = None
        if feature.form_hint in {"slit", "capsule"}:
            requested_radius = max(0.5 * height, 0.5 * minimum_feature_mm)
            radius = min(requested_radius, 0.45 * width)
            half_segment = max(0.0, 0.5 * width - radius)
            base.update(kind="capsule", start=[-half_segment, 0.0, 0.0], end=[half_segment, 0.0, 0.0], radius_mm=radius)
            effective_depth = min(maximum_relief_depth_mm, max(depth, 1.01 * minimum_relief_depth_mm))
            transform = {"scale": [1.0, effective_depth / radius, 1.0]}
            depth_scale = min(1.0, effective_depth / radius)
            base["blend_mm"] = max(float(base["blend_mm"]), 1.01 * minimum_blend_mm / depth_scale)
        elif feature.form_hint in {"disc", "oval"}:
            base.update(kind="ellipsoid", center=[0.0, 0.0, 0.0], radii=[0.5 * width, depth, 0.5 * height])
        elif feature.form_hint in {"leaf", "point"}:
            base.update(kind="rounded_triangle_prism", vertices_xz=[[-0.5 * width, -0.5 * height], [0.5 * width, -0.5 * height], [0.0, 0.5 * height]], center_y_mm=0.0, half_depth_mm=depth, round_mm=max(0.4, 0.25 * minimum_feature_mm))
        elif feature.form_hint == "arch":
            base.update(kind="arched_prism", center=[0.0, 0.0, 0.0], bottom_z_mm=-0.5 * height, spring_z_mm=0.05 * height, half_width_mm=0.5 * width, half_depth_mm=depth, round_mm=max(0.4, 0.25 * minimum_feature_mm))
        else:
            ...
            base.update(kind="rounded_box", center=[0.0, 0.0, 0.0], half_sizes=half_sizes, round_mm=round_mm)
