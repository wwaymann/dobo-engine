...
from .continuous_morphological_fusion import CONTINUOUS_MORPHOLOGICAL_FUSION_VERSION
...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the DOBO Macroblock A.3 real visual matrix."
    )
...
def main() -> None:
    ...
    print()
    print("DOBO Macroblock A.3 Real Matrix")
    print("Continuous Morphological Fusion", CONTINUOUS_MORPHOLOGICAL_FUSION_VERSION)
    print("-----------------------------------")
    for case in cases:
        result = DoboStructuralPipeline().generate_from_semantic(
            case.program,
            output_root=root / case.id,
            interpreter_version="macroblock-a3-matrix",
            model="not-used",
    ...
    print("DOBO Macroblock A.3 Real Matrix: Valid OK")
