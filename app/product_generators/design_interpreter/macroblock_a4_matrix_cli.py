...
from .visible_morphological_continuity import VISIBLE_MORPHOLOGICAL_CONTINUITY_VERSION


def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the DOBO Macroblock A.4 real visual matrix."
    )
...
def main() -> None:
    ...
    print()
    print("DOBO Macroblock A.4 Real Matrix")
    print("Visible Morphological Continuity", VISIBLE_MORPHOLOGICAL_CONTINUITY_VERSION)
    print("-----------------------------------")
    for case in cases:
        result = DoboStructuralPipeline().generate_from_semantic(
            case.program,
            output_root=root / case.id,
            interpreter_version="macroblock-a4-matrix",
            model="not-used",
    ...
    print("DOBO Macroblock A.4 Real Matrix: Valid OK")
