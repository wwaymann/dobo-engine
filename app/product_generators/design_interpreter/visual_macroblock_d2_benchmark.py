...
BENCHMARK_VERSION="D2.0-consolidation"
def _mesh(path):
...
def run(output_root):
    ...
    print("D2 CONSOLIDATION PASS",passed,"FAIL",len(records)-passed); print("manifest",manifest)
    if passed!=len(records): raise SystemExit(1)
    return payload
if __name__=="__main__": run("outputs/consolidation_macroblock_d2")
