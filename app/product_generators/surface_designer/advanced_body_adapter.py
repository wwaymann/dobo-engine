...
def local_surface_mapping_proxy(
    ...
    *,
    ...
    Imported STL bodies expose every triangle as an OCC face. ``faceOn`` then
    scales a complete text operation into one tiny triangle, which is not the
    intended SurfaceDesigner contract. This helper derives one local tangent
    mapping plane from the source mesh itself and places it slightly inside the
    existing body so the unchanged emboss Boolean has a shared volume.

    The proxy is used only as a parameterization/mapping interface. It is never
    substituted for, fused into, exported as, or otherwise used to replace the
    advanced product body.
    """
    source = Path(path).resolve()
    if not source.is_file() or source.stat().st_size <= 0:
        raise FileNotFoundError(source)
    ...
    # Prefer the middle external wall, with an outward radial normal and low
    # vertical normal component. Area is only a tie-breaker because the source
    # remains intentionally triangulated.
    eligible = (
        (z_fraction >= 0.38)
        & (z_fraction <= 0.68)
