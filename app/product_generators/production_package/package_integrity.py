...
class PackageIntegrityVerifier:
    ...
    def _manifest_from_json(payload: dict[str, object]) -> GenerationPackageManifest:
        ...
        return GenerationPackageManifest(
            ...
            motor_version=str(payload["motor_version"]),
            source_revision=str(payload["source_revision"]),
            source_specification=str(payload["source_specification"]),
            source_sha256=str(payload["source_sha256"]),
            render_contract_version=str(payload["render_contract_version"]),
            artifacts=records,
    ...
    def verify(cls, package_root: str | Path) -> PackageIntegrityReport:
        ...
        if manifest.schema_version != ProductionPackageBuilder.SCHEMA_VERSION:
            raise RuntimeError(
                ...
                f"expected {ProductionPackageBuilder.SCHEMA_VERSION!r}."
            )

        copied_source = root / "source" / Path(manifest.source_specification).name
        source_verified = (
            copied_source.is_file()
            and copied_source.stat().st_size > 0
            and cls._sha256(copied_source) == manifest.source_sha256
        )
        ...
        for record in manifest.artifacts:
            ...
            if (
                ...
                or materialized.stat().st_size != record.size_bytes
                or cls._sha256(materialized) != record.sha256
            ):
        ...
        identity_verified = (
            root.name == manifest.package_sha256
            and expected_identity == manifest.package_sha256
        )
        report = PackageIntegrityReport(
            package_root=str(root),
            package_sha256=manifest.package_sha256,
            source_verified=source_verified,
            artifact_count=verified_count,
