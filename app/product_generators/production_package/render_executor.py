...
class DeterministicRenderExecutor:
    ...
    def _basis(direction):
        ...
        up=np.asarray((0.,0.,1.))
        if abs(float(np.dot(forward,up)))>.98: up=np.asarray((0.,1.,0.))
    ...
    def execute(cls, mesh_path:str|Path, contract:RenderContract, output_directory:str|Path)->tuple[RenderArtifact,...]:
        ...
        if not isinstance(mesh,trimesh.Trimesh) or len(mesh.vertices)==0: raise RuntimeError("Render source mesh is empty.")
        vertices=np.asarray(mesh.vertices,dtype=float); faces=np.asarray(mesh.faces,dtype=int)
        vertices-=.5*(vertices.min(0)+vertices.max(0)); root=Path(output_directory); result=[]
        for view in contract.views:
            right,up,forward=cls._basis(view.direction); x=vertices@right; y=vertices@up; z=vertices@forward
            span=max(float(np.ptp(x)),float(np.ptp(y)),1e-9); scale=.82*min(view.width_px,view.height_px)/span
            px=view.width_px*.5+x*scale; py=view.height_px*.5-y*scale
            mode="RGBA" if view.transparent_background else "RGB"; background=(255,255,255,0) if mode=="RGBA" else (245,245,245)
            image=Image.new(mode,(view.width_px,view.height_px),background); draw=ImageDraw.Draw(image); edge=(70,70,70,255) if mode=="RGBA" else (70,70,70)
            ...
            path=root/f"{view.name}.png"; path.parent.mkdir(parents=True,exist_ok=True); image.save(path,format="PNG",compress_level=9)
