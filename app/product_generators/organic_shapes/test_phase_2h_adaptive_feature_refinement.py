...
SPEC_PATH = Path(__file__).with_name("phase_2h_adaptive_feature_refinement.json")


def main() -> None:
    print()
    print("DOBO Organic Shape Engine - Phase 2H")
    print("Feature size/SDF proximity -> adaptive local density -> refined STL")
    print("-----------------------------------")
    ...
    diagnostics = engine.adaptive_refinement_diagnostics(specification, result.mesh)
    ...
    print(
        "adaptive placements selected",
        diagnostics["placements_selected"],
        "OK",
    )
    print("small feature placements", diagnostics["small_placements"], "OK")
    print(
        "exclusive detail subdivision passes",
        specification.adaptive_refinement.detail_subdivision_passes,
        "OK",
    )
    print("feature vertices", diagnostics["feature_vertices"], "OK")
    print("feature edges", diagnostics["feature_edges"], "OK")
    print(
        "feature median edge mm",
        round(diagnostics["feature_median_edge_mm"], 4),
        "OK",
    )
    print(
        "background median edge mm",
        round(diagnostics["background_median_edge_mm"], 4),
        "OK",
    )
    if diagnostics["placements_selected"] != len(placements):
        raise RuntimeError("Adaptive refinement did not locate every placement.")
    if diagnostics["small_placements"] < 5:
        raise RuntimeError("Adaptive refinement did not identify small features.")
    if diagnostics["feature_vertices"] < 1000:
        raise RuntimeError("Adaptive refinement selected insufficient feature density.")
    if not (
        diagnostics["feature_median_edge_mm"]
        < 0.45 * diagnostics["background_median_edge_mm"]
    ):
        raise RuntimeError("Adaptive feature edges are not materially denser.")
    ...
    print(
        "feature active refinement vertices",
        result.mesh_quality.feature_active_vertex_count,
        "OK",
    )
    if result.mesh_quality.feature_active_vertex_count < 1000:
        raise RuntimeError("Mesh refiner did not activate the feature regions.")
    print(
    ...
    if result.generation_seconds > result.max_generation_seconds:
        raise RuntimeError("Adaptive refinement exceeded the Phase 2H budget.")
    print("generation budget OK")
    print("DOBO Organic Shape Engine Phase 2H: Valid OK")
