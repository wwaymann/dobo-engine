...
def build_drain(
    ...
    """Agrega el drenaje al cuerpo paramétrico de la maceta."""
    if model is None:
        raise ValueError("build_drain requires an existing model.")
    ...
    model = model.faces("<Z").workplane().hole(drain_diameter)
    context.add_operation("build_drain")
    return model
